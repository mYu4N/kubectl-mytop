...
import (
	...
	mytop "github.com/chlyyangwei/kubectl-mytop/pkg/mytop"
...
var rootCmd = &cobra.Command{
	Use:   "kubectl-mytop",
	Short: "kubectl-mytop provides an overview of the resource requests, limits, and utilization in a Kubernetes cluster.",
	Long:  "kubectl-mytop provides an overview of the resource requests, limits, and utilization in a Kubernetes cluster.",
	Run: func(cmd *cobra.Command, args []string) {
		...
		mytop.FetchAndPrint(showContainers, showPods, showUtil, showPodCount, availableFormat, podLabels, nodeLabels,
...
func init() {
	...
	rootCmd.PersistentFlags().StringVarP(&sortBy,
		"sort", "", "name",
		fmt.Sprintf("attribute to sort results be (supports: %v)", mytop.SupportedSortAttributes))

	rootCmd.PersistentFlags().StringVarP(&outputFormat,
		"output", "o", mytop.TableOutput,
		fmt.Sprintf("output format for information (supports: %v)", mytop.SupportedOutputs()))
}
...
func validateOutputType(outputType string) error {
	for _, format := range mytop.SupportedOutputs() {
		if format == outputType {
			return nil
		}
	}
	return fmt.Errorf("Unsupported Output Type. We only support: %v", mytop.SupportedOutputs())
}
